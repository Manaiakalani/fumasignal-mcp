...
function registerTools(server: McpServer, source: FumadocsSource): void {
  ...
  server.registerTool(
    'search_docs',
    {
      ...
      inputSchema: {
        ...
        locale: z.string().trim().max(100).optional().describe('Optional locale filter (e.g. "en", "cn").'),
      },
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    'get_section',
    {
      ...
      inputSchema: {
        ref: z.string().trim().min(1).max(2000).describe('URL path, absolute URL, or slug of the page.'),
        anchor: z
          .string()
          .trim()
          .min(1)
